...
| `curl` / `fatmax hook` が大量 | ウォッチャーが動いておらず、hook が全部ハブ直送になっています。段4へ |
...
**止めている間も記録は消えません**（hook がハブへ直に送り、送れなかったぶんは
次の hook が持って行きます）。切り分けが済んだら戻してください。
...
**違っていたら両方を 0.1.84 以上へ。片方だけでは直りません。**
...
0.1.84 以降は、①古い名前でも訊き直す ②空振りしたら間隔を倍にする（1秒 → 最大60秒）
