...
3台で 8 セッション回していると、どれが自分の返事を待って止まっているのか分からなくなります。
気づいたときには、1本が 40 分止まっていた。fatmax は**それを1画面に集めて、音で知らせます**。

![止まっているセッションが一覧に出ている画面](img/notify.png)

- **赤は「行かないと止まったまま」。** 実行許可と質問への回答だけを赤にします。
  返答済みで次の入力を待っているだけのものは灰色。**行くべきかどうかが色で決まります**
- **音でも鳴ります。** 画面を見ていなくても気づけるように、赤は約1.8秒くり返し。
  用途ごとに音色を選べて、「鳴らさない」も選べます
- **会話がそのまま読めます。** どのマシンのセッションでも、質問と選んだ答え、
  承認した計画、実行中に差し込んだ指示まで
- **1日の帯。** その日に動いた全セッションを横並びにして、
  **人間待ちが何%だったか**を出します
- **コスト。** マシン別・案件別・モデル別・日別。今日の合計が $50 / $100 / $150 を
  超えたら知らせます

![会話がそのまま読める画面](img/talk.png)

## しくみ

```mermaid
flowchart LR
  subgraph M["見たいマシン（何台でも）"]
    direction TB
    A["Mac<br/>Claude Code ×3"]
    B["Windows<br/>Claude Code ×2"]
    C["Docker コンテナ<br/>Claude Code"]
  end
  H["fatmax hub<br/>実行ファイル1個<br/>画面も記録も内包"]
  V["ブラウザ<br/>⚠ 許可待ち / 💬 入力待ち / ● 実行中"]
  A -->|"hook: curl 1行"| H
  B -->|"hook: curl 1行"| H
  C -->|"hook: curl 1行"| H
  H -->|":8787"| V
...
**監視される側に置くものはありません。** `~/.claude/settings.json` に curl の1行が入るだけで、
常駐するものも、配るファイルもありません。抜くのも1コマンドです。

集約する1台だけが `fatmax hub` を動かします。**依存ゼロの実行ファイル1個**で、
UI も SQLite も内包しています。
