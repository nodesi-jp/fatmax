...
**「10日前のあのセッションで、何を指示して何が返ってきたか」を引けます。**
...
**探す単位はセッションではなく発話**（1行の発言）です。セッションを返されても
中をもう一度探す羽目になるので、当たった発言そのものへ着地します。

### 実際の画面

![検索画面。「タイムライン」で 88 件の発話が当たり、当たった箇所が黄色く強調されている](img/search-live.png)

- **一覧の星や非表示では絞りません。** 隠した会話も、終わった会話も必ず見つかります
- **日付も既定では絞りません。** 探す人は「いつの話か」も覚えていないから探しています
  （対象日を入れたときだけ効きます）
- **全角/半角・ひらがな/カタカナのゆれを畳んでから当てます。** `Ｂａｓｈ` でも `ふぁとまっくす` でも当たります
- 全部の語が当たらなくても出しますが、点数で下に置き、行に「2語中1語」と書きます
- `#search:<語>` で、探した状態の URL を貼れます
