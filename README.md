...
- **止まっているものが分かる。** 画面を見ていなくても、音で気づけます
- **会話がそのまま読める。** どのマシンのセッションでも、質問と選んだ答え、
  承認した計画、実行中に差し込んだ指示まで
- **1日の帯。** その日に動いた全セッションを横並びにして、**人間待ちが何%だったか**
- **コスト。** マシン別・案件別・モデル別・日別。今日の合計が $50 / $100 / $150 を
  超えたら知らせます
- **見たいマシンに置くものは無い。** `settings.json` に curl の1行が入るだけ
- **集約する1台も実行ファイル1個。** 依存ゼロ。UI も SQLite も内包しています
...
**セッションが何個あっても1画面**に集まります。中央は実際の画面です。
